...
int ll_remove(LinkedList* this,int index)//análoga a addNode
{
    ...
    if(this != NULL)
    {
    	...
    	if(index>=0 && index<lenLinkedList)
    	{
    		...
    		free(pNodoABorrar);//libero la memoria reservada del nodo que borre
    		this->size--;
    		returnAux = 0;
    	}
    }
    ...
    if(this != NULL && pFunc != NULL && (order == 1 || order == 0))
    {
    	...
    	for(int i=0; i<lenLinkedList-1; i++)
    	{
    		for(int j=i+1; j<lenLinkedList; j++)
    		{
    			aux1 = ll_get(this, i);
    			aux2 = ll_get(this, j);
    			retornoFunc = pFunc(aux1, aux2);
    			if(((order == 1) && (retornoFunc==1)) //ascendente - aux1>aux2 y necesito aux1<aux2
    				||((order == 0) && (retornoFunc==-1))) //descendente - aux1<aux2 y necesito aux1>aux2
    			{
